# Run menu regression checks against the built plugin in a fresh, disposable working directory.
...
$testRoot = Join-Path $repo "build/pure-map-tests/$([guid]::NewGuid())"
New-Item -ItemType Directory -Force "$testRoot/classes" | Out-Null
$classpath = "$repo/build/classes/java/main;$dependency"
& "$jdk/bin/javac.exe" -encoding UTF-8 -sourcepath "$testRoot/classes" -cp $classpath -d "$testRoot/classes" "$PSScriptRoot/PureMapsTest.java"
if ($LASTEXITCODE -ne 0) { throw 'Test compilation failed.' }
Push-Location $testRoot
try {
    & "$jdk/bin/java.exe" -cp "$testRoot/classes;$classpath" Extinction.PureMapsTest
    if ($LASTEXITCODE -ne 0) { throw 'Pure map checks failed.' }
} finally {
    Pop-Location
}
