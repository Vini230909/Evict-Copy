...
Server-side Mindustry plugin (game v157.4, Java 17 source) for Evict-style persistent PvP on a procedurally generated hex map. Runs on a dedicated server; clients install nothing. Current version **1.6.0** — `plugin.json` `version` and the startup revision string in `EvictMapPlugin` must always match.
...
- `moderation/` — ban widening and propagation (see Bans): `BanCascade` (pure one-step expansion: an account's own IPs, never onward to other accounts), `BanManager` (hub: event hooks, applying, kicks, one-off import), `BanList` (the file workers read), `BanSync` (worker: applies the hub's list), `BanReport` (what one action hit).
