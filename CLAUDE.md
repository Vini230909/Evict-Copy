...
- `moderation/` — ban widening and propagation (see Bans): `BanCascade` (pure one-step expansion: an account's own IPs, never onward to other accounts), `BanManager` (hub: event hooks, applying, kicks, one-off import), `BanList` (the file workers read), `BanSync` (worker: applies the hub's list), `BanReport` (what one action hit). Plus the word filter (see Word filter): `BannedWords` (**the word list — the only file to edit to add a word**), `WordMatcher` (pure folding + matching), `WordFilter` (the chat filter and name check that ban), `WordFilterHit` (what one hit was: word, chat-vs-name, the text, the console timestamp and which console).
...
- Handshake `duel.properties` (hub → worker): `mode`, `team.count`, `team.N` UUID lists. Result `result.properties` (worker → hub): `mode`, `winner.uuid(s)`, `loser.uuid(s)`, `reason`. Status `status.properties` (worker → hub, rewritten every 2 s): state, game time, connected players, `out`/`participants`/`owner`, `playtime` (`uuid:millis` running totals), `banrequests` (accounts the word filter caught, for the hub to ban) and one `banrequest.<uuid>.source|word|text|time` block per request (what the filter saw, so the hub's ban log can name it and point at this worker's console log). Only `ranked` results record as ranked; `1v1`/`teams`/`ffa` become unranked `/history` entries; Training/Sandbox are never recorded.
...
- `/ban [name]` — admin-only chat command with the `/info` UX: no argument opens a picker of the online players (never the admin themself); a name searches the stored players (partial latest-name first, so it works on someone who already left), several matches open the picker. Always a named confirmation menu before anything happens. It only seeds `banPlayerID`, so it is widened, kicked, synced and logged like every other path. On a worker it refuses and points at the hub. Listed in `/help`, gated at execution.
...
- Ban log: `evictbanlog <url>` posts a new message per action into its own channel — never an edit, a log is a sequence. One embed with all names, all UUIDs, all IPs the action covered. A word-filter ban gets its own entry (orange, `🤖 Word filter ban`) with four extra fields above those: the word that tripped, whether it stood in the **chat message** or the **player name**, the offending text itself (the whole chat message, or the name), and the **console time** in the server's own log format (`MM-dd-yyyy HH:mm:ss`) plus which console wrote it — the hub's, or a match server named by port — so the incident can be found again by searching that log. **Staff-only channel**: it publishes addresses. Messages are queued and released one per 1.5 s (Discord rate limits a webhook at a handful of requests a second); an import of more than 25 old bans collapses into summary embeds of 20 instead of flooding the channel.
...
- The hub bans through `banPlayerID` — widened, kicked, synced and logged like an admin's, but the ban log entry is marked as the filter's and carries the word, where it stood (chat message vs. player name), the offending text and the console timestamp (see Bans). A worker never bans: it kicks and publishes the account plus that same detail in `status.properties` (`banrequests` + `banrequest.<uuid>.*`) for the hub's next poll, which logs it against the worker's port and its console time.
