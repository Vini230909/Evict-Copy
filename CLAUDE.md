...
Current version **1.15.10** — `plugin.json` `version` and the startup revision string
...
re-copies `config/mods` into its folder and `WorkerFolder.refreshJars()` updates stale
