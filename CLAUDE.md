...
Server-side Mindustry plugin (game v157.4, Java 17 source) for Evict-style persistent PvP on a procedurally generated hex map. Runs on a dedicated server; clients install nothing. Current version **1.10.0** — `plugin.json` `version` and the startup revision string in `EvictMapPlugin` must always match.
...
- `duel/WorkerConsole` — hub-side console attach (`evictattach`, see Worker console).
...
- Handshake `duel.properties` (hub → worker): `mode`, `team.count`, `team.N` UUID lists. Result `result.properties` (worker → hub): `mode`, `winner.uuid(s)`, `loser.uuid(s)`, `reason`. Chat `chat.log` (worker → hub, append-only): the worker's captured chat/command/join/leave lines for the Discord chat mirror, tailed by the hub and deleted at spawn (see Discord chat mirror). Status `status.properties` (worker → hub, rewritten every 2 s): state, game time, connected players, `out`/`participants`/`owner`, `playtime` (`uuid:millis` running totals), `banrequests` (accounts banned on the worker, for the hub to apply) and one `banrequest.<uuid>.actor|time` block per request, plus `source|word|text` when the word filter decided it — so the hub's ban log can name who banned and point at this worker's console log. Only `ranked` results record as ranked; `1v1`/`teams`/`ffa` become unranked `/history` entries; Training/Sandbox are never recorded.
...
### Worker console
The hub console can be sent to a running match server and back: `evictattach <port>` attaches, `evictattach` alone (or `evictdetach`) comes back. While attached, every line typed goes to that worker's console and everything the worker prints is relayed into the hub console, prefixed `[<port>]`. There is no other way in — a worker has no terminal of its own (its output is redirected into `duel-workers/duel-<port>/worker.log`).
- **Input** rides the stdin pipe the hub already uses for `config port` / `host`, kept open in the worker handle for the worker's lifetime. **Output** is the `worker.log` tail, read from end-of-file at attach every 200 ms, printed raw (`System.out`) so the worker's own timestamps, level tags and colours survive; what was *typed* is logged normally, so the hub's log keeps the audit trail.
- The stdin loop is intercepted by replacing `ServerControl.serverInput` in `bootstrap()` — a plain mutable field that `ServerLauncher` only starts on `ServerLoadEvent`, two lines after the `mods.eachClass(Mod::init)` that runs `bootstrap()`. The plugin is always first and vanilla's loop never starts; there is exactly one stdin reader either way. Installed on the **hub only**, so the worker's proven `config`/`host` injection path stays vanilla.
- **Not getting stuck is the design constraint.** The keyword is matched on the raw line before anything is forwarded, so it is never sent to the worker and the way back never depends on the worker, the hub's command handler or the game loop. Attach/detach run on the console thread itself, not as a posted main-thread task. Any exception in the relay detaches and keeps the console reading. A worker that exits (or a broken pipe) detaches out loud, and a stale tail thread can only detach the attachment it belongs to. `evictattach` is also registered on the **worker**, where it answers that attaching works from the hub console only — a stray line there says where it is instead of `Invalid command`.
- `evictattach` with no argument while not attached lists the attachable ports instead of doing nothing. Attaching to another port while attached switches directly.
- Everything typed goes through, including `exit`, `stop` and `host` — those end the running match. That is the point of a console; there is no allow-list.
...
- Keys in use: `DISCORD_CHAT_BOT_TOKEN` (chat mirror). Reload without a restart: `evictchatlog reload`.
...
- Word filter: `evictwordfilter [on/off/test <text>]` — no argument shows whether it is on and how many words it watches; `on`/`off` switch it (stored as `moderation.wordFilter`, default on, copied into every worker with the rest of the settings file); `test <text>` says which entry a line would trip. The words themselves are in the source, not here (see Word filter).
...
- Worker console: `evictattach [port]` — sends the console to that match server; no argument comes back (or lists the running ports when already on the hub). `evictdetach` is a detach-only spelling, registered as its own command so `help` lists the way back too. Both sit near the bottom of `help`: Mindustry registers mod commands last. See Worker console.
