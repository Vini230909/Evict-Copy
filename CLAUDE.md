...
Server-side Mindustry plugin (game v157.4, Java 17 source) for Evict-style persistent PvP on a procedurally generated hex map. Runs on a dedicated server; clients install nothing. Current version **1.7.0** — `plugin.json` `version` and the startup revision string in `EvictMapPlugin` must always match.
...
- The hub bans through `banPlayerID` — widened, kicked, synced and logged like an admin's. A worker never bans: it kicks and publishes the account in `status.properties` (`banrequests`) for the hub's next poll.
